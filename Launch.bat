...
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "from pathlib import Path; from programs.shared_config import load_config; c=load_config(Path('config.json')); print(c.get('main_script','programs/check_source_and_extract_to_output.py'))"`) do set "MAIN_SCRIPT=%%I"
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "from pathlib import Path; from programs.shared_config import load_config; c=load_config(Path('config.json')); print(c.get('flatten_script','programs/flatten_single_nested_mod_folder.py'))"`) do set "FLATTEN_SCRIPT=%%I"
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "from pathlib import Path; from programs.shared_config import load_config; c=load_config(Path('config.json')); print(c.get('rename_script','programs/rename_duplicate_mod_folders.py'))"`) do set "RENAME_SCRIPT=%%I"
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "from pathlib import Path; from programs.shared_config import load_config; c=load_config(Path('config.json')); print(c.get('validate_script','programs/validate_output_structure.py'))"`) do set "VALIDATE_SCRIPT=%%I"
