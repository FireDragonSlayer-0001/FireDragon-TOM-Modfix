...
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "import json; c=json.load(open('config.json','r',encoding='utf-8')); n=lambda s:''.join(ch for ch in s.lower() if ch.isalnum()); m={n(k):v for k,v in c.items()} if isinstance(c,dict) else {}; print(c.get('alternative_output_dir', m.get('alternativeoutputdir','Alternative Output')) if isinstance(c,dict) else 'Alternative Output')"`) do set "ALTERNATIVE_OUTPUT_DIR=%%I"
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "import json; c=json.load(open('config.json','r',encoding='utf-8')); n=lambda s:''.join(ch for ch in s.lower() if ch.isalnum()); m={n(k):v for k,v in c.items()} if isinstance(c,dict) else {}; print(c.get('programs_folder', m.get('programsfolder','programs')) if isinstance(c,dict) else 'programs')"`) do set "PROGRAMS_FOLDER=%%I"
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "import json; c=json.load(open('config.json','r',encoding='utf-8')); n=lambda s:''.join(ch for ch in s.lower() if ch.isalnum()); m={n(k):v for k,v in c.items()} if isinstance(c,dict) else {}; print(c.get('manual_fixing_required_dir', m.get('manualfixingrequireddir','Manual Fixing Required')) if isinstance(c,dict) else 'Manual Fixing Required')"`) do set "MANUAL_FIXING_DIR=%%I"
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "import json; c=json.load(open('config.json','r',encoding='utf-8')); n=lambda s:''.join(ch for ch in s.lower() if ch.isalnum()); m={n(k):v for k,v in c.items()} if isinstance(c,dict) else {}; v=(c.get('auto_update_tools') if isinstance(c,dict) else None); v=m.get('autoupdatetools', v); s=str(v).strip().lower() if v is not None else ''; print('True' if (v is True or (isinstance(v,(int,float)) and v!=0) or s in ('1','true','yes','y','on')) else 'False')"`) do set "AUTO_UPDATE_TOOLS=%%I"
