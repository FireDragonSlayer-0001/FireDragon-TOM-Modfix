...
for /f "usebackq tokens=*" %%I in (`%PYTHON_CMD% -c "import json; c=json.load(open('config.json','r',encoding='utf-8')); n=lambda s:''.join(ch for ch in s.lower() if ch.isalnum()); m={n(k):v for k,v in c.items()} if isinstance(c,dict) else {}; print(c.get('alternative_builder_script', m.get('alternativebuilderscript','programs/build_alternative_output_for_broken_mods.py')) if isinstance(c,dict) else 'programs/build_alternative_output_for_broken_mods.py')"`) do set "ALT_SCRIPT=%%I"
